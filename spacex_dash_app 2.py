...
def get_pie_chart(entered_site):
    new_df = spacex_df.groupby(['Launch Site'], as_index=False).mean()
    if entered_site == 'ALL':
       return px.pie(new_df, values='class', names='Launch Site', title='Launch Success Rate For All Sites')
       
    # return the outcomes in pie chart for a selected site
    new_df = spacex_df[spacex_df['Launch Site'] == entered_site]
    new_df['outcome'] = new_df['class'].apply(lambda x: 'Success' if (x == 1) else 'Failure')
    new_df['counts'] = 1
    return px.pie(new_df, values='counts', names='outcome', title='Launch Success Rate For ' + entered_site)
...
def get_scatter_chart(entered_site, slider):
    ...
    # return the outcomes in pie chart for a selected site
    new_df = new_df[new_df['Launch Site'] == entered_site]
    new_df['outcome'] = new_df['class'].apply(lambda x: 'Success' if (x == 1) else 'Failure')
    new_df['counts'] = 1
    return px.scatter (new_df,
                       x='Payload Mass (kg)', y='class',
